...
namespace lgu3d
{
    ...
    /// CS服务模块 UDP 组件
    /// </summary>
    public class ServiceModuleUdpComp : ModelCompBase<ServiceModule>
    {
        #region 框架构造
        ...
        void EndReceiveFromCallback(IAsyncResult iar)
        {
            ...
            try
            {
                ...
                MyModule.RecvMessage(state.Buffer);
            }
            catch (Exception e)
            {
        ...
        {
            EndPoint remoteEP = new IPEndPoint(IPAddress.Parse(MyModule.ip), MyModule.udpprot);
